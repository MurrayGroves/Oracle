...
async def whileLoop():
    while True:
        ...
        if d.entries[0].title not in latestEp:
            ...
            em = discord.Embed(
                title=d.entries[0].title, description=cleantext, colour=13124085
            )
...
async def cmd_learn(message):
    ...
    f = open(f"data/{message.guild.id}/commands.json")
    commands = json.load(f)
    f.close()
    commands.update({trigger: action})
    f = open(f"data/{message.guild.id}/commands.json", "w")
...
async def cmd_unlearn(message):
    ...
    em = discord.Embed(
        title="Unlearn", description=msg, colour=random.randint(0, 1677215)
    )
    await message.channel.send(embed=em)
...
async def on_message(message):
    ...
    try:
        f = open(f"data/{message.guild.id}")
        f.close()
